...
def dem_file_upload(
    *,
    ...
    transaction.on_commit(
        lambda: run_dem_validation_task.delay(dem.id)
    )

    return dem


# apps/satellite/services.py

@transaction.atomic
def icesat2_job_submit(
    *,
    basin_id: int,
    submitted_by,
) -> IceSat2Job:
    from apps.satellite.selectors import basin_get_by_id

    basin = basin_get_by_id(basin_id=basin_id)
    ...
    transaction.on_commit(
        lambda: process_icesat2_job_task.delay(job.id)
    )
...
| ORM: `full_clean`, `save`, транзакції | [Django_ORM.md](Django_ORM.md) |
| Views: як передати `validated_data` у Service | [Django_Views.md](Django_Views.md) |
