...
## Приклади для GeoHydroAI

```python
# apps/dem/selectors.py

def dem_get_best_for_region(*, region_id: int) -> QuerySet:
    return (
        DemFile.objects
        .filter(region_id=region_id, is_validated=True)
        .select_related('region')
        .order_by('-resolution', '-created_at')
    )


# apps/satellite/selectors.py

def icesat2_tracks_get_for_bounds(*, bbox) -> QuerySet:
    from django.contrib.gis.geos import Polygon
    geom = Polygon.from_bbox(bbox)
    return (
        IceSat2Track.objects
        .filter(geometry__intersects=geom)
        .select_related('job')
        .prefetch_related('points')
    )


# apps/papers/selectors.py

def papers_get_unvalidated(*, limit: int = 50) -> QuerySet:
    return (
        Paper.objects
...
| Тема | Файл |
|------|------|
| Services (бізнес-логіка, мутації, транзакції) | [DJANGO_SERVICES_SELECTORS.md](DJANGO_SERVICES_SELECTORS.md) |
| ORM: QuerySet API, N+1, `select_related` | [Django_ORM.md](Django_ORM.md) |
| Views: як викликати Selector з View | [Django_Views.md](Django_Views.md) |
